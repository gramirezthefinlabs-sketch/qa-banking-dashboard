...
attention_rate = (failed + blocked) / len(executed) if len(executed) else 0
unique_defects = filtered["defecto_id"].dropna().nunique()
defect_rate = unique_defects / len(executed) if len(executed) else 0
...
with row1[3]:
    metric_card(
        ...
        f"{attention_rate:.1%}",
        "Casos fallidos y bloqueados / casos ejecutados",
    )
with row1[4]:
    metric_card(
        "Tasa de defectos",
        f"{defect_rate:.1%}",
        "Tickets de defecto únicos / casos ejecutados",
    )
...
with tab_summary:
    ...
    with left:
        ...
        status_counts["Estado"] = pd.Categorical(
            status_counts["Estado"], categories=ordered, ordered=True
        )
    ...
    st.markdown("#### Estado de los casos por prioridad")
    priority_order = list(PRIORITY_LABELS.values())
    priority_colors = {
        "1-Crítica": "#F4B223",
        "2-Alta": "#0B5D7A",
        "3-Media": "#5B93A8",
    ...
    status_priority_chart.update_traces(
        textposition="outside",
        hovertemplate=(
            "<b>%{x}</b><br>Prioridad: %{fullData.name}<br>"
            "Casos: %{y}<extra></extra>"
        ),
    )
    status_priority_chart.update_layout(
        height=390,
        xaxis_title="Estado del caso",
        yaxis_title="Cantidad de casos",
        legend_title_text="Prioridad",
        margin=dict(l=10, r=10, t=10, b=10),
    )
    st.plotly_chart(status_priority_chart, use_container_width=True)
